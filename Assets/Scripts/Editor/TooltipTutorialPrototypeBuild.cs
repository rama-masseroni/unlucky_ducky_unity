...
public static class TooltipTutorialPrototypeBuild
{
    private const string OutputPath = "Builds/TooltipTutorialPrototype/UnluckyDucky_TooltipTutorial.exe";
    ...
    [MenuItem("Unlucky Ducky/Build/Tooltip Tutorial Prototype (Windows)")]
    public static void BuildWindows()
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException($"Tooltip tutorial build failed: {report.summary.result}");
            }

            Debug.Log($"Tooltip tutorial build created at {Path.GetFullPath(OutputPath)}");
        }
        finally
        {
    ...
    private static void ApplyNextScenes()
    {
        foreach (NextSceneOverride sceneOverride in NextSceneOverrides)
        {
            ...
            SerializedProperty nextScene = serializedDefinition.FindProperty("nextSceneName");

            if (nextScene == null)
            {
                throw new InvalidOperationException($"Missing nextSceneName in: {sceneOverride.AssetPath}");
            }

            nextScene.stringValue = sceneOverride.NextSceneName;
